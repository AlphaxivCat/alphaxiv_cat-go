...
func TestUserGetPrivateNotesWithOptionalParams(t *testing.T) {
	...
	_, err := client.Users.GetPrivateNotes(
		context.TODO(),
		"uid",
		alphaxivcat.UserGetPrivateNotesParams{
...
func TestUserWeighWeeklyReputation(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Users.WeighWeeklyReputation(context.TODO())
	if err != nil {
		var apierr *alphaxivcat.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
