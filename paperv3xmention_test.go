...
func TestPaperV3XMentionUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.Papers.V3.XMentions.Update(
		...
		alphaxivcat.PaperV3XMentionUpdateParams{
...
func TestPaperV3XMentionDelete(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	err := client.Papers.V3.XMentions.Delete(context.TODO(), "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
	if err != nil {
		var apierr *alphaxivcat.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
